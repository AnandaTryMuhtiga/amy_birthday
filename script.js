class Calculator {
  constructor(expressionDisplay, valueDisplay) {
    ...
    this.loveSound = new Audio('./amyy.mp3');
  ...
  calculate() {
    try {
      ...
      if (expression.replace(/\s/g, "").replace(/÷/g, "/") === "20-5+9/9+1") {
        ...
        setTimeout(() => {
  this.valueDisplay.innerHTML = "Happy 17th Birthday Amy";
  this.valueDisplay.classList.add("result", "heart-message");
  this.valueDisplay.classList.remove("glitch-error");
  this.valueDisplay.setAttribute("aria-label", "Happy birthday message");

  this.history.push(`${expression} = HAPPY BIRTHDAY AMY`);
  ...
  displayHistory() {
    ...
    historyDiv.querySelectorAll("p").forEach(p => {
      p.addEventListener("click", () => {
        ...
        if (p.dataset.result === "HAPPY BIRTHDAY AMY") {
  this.valueDisplay.innerHTML = "happy birthday amy";
  this.valueDisplay.classList.add("result", "heart-message");
  this.valueDisplay.setAttribute("aria-label", "Happy birthday message");
  this.playLoveSound();
} else {
